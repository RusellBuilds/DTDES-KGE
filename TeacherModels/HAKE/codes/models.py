...
class KGEModel(nn.Module, ABC):
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        if not args.CRRLoss:
            ...
            pos_score_for_crr = -positive_score
            neg_score_for_crr = -negative_score
            
            diff = pos_score_for_crr - neg_score_for_crr
            exponent = torch.clamp((p - diff) / t, min=-20, max=20)
            cliff_sigmoid_values = 1 / (1 + torch.exp(exponent))
            
            sum_of_cliffs = torch.sum(cliff_sigmoid_values, dim=1)
...
class ModE(KGEModel):
    def __init__(self, args, num_entity, num_relation, hidden_dim, gamma):
        super(ModE, self).__init__()
        self.args = args
        self.num_entity = num_entity
        self.num_relation = num_relation * 2
        ...
        self.relation_embedding = nn.Parameter(torch.zeros(self.num_relation, hidden_dim))
        nn.init.uniform_(
...
class HAKE(KGEModel):
    def __init__(self, args, num_entity, num_relation, hidden_dim, gamma, modulus_weight=1.0, phase_weight=0.5):
        ...
        self.relation_embedding = nn.Parameter(torch.zeros(self.num_relation, hidden_dim * 3))
        nn.init.uniform_(
            tensor=self.relation_embedding,
            a=-self.embedding_range.item(),
            b=self.embedding_range.item()
