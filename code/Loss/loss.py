...
class SigmoidLoss(nn.Module):
    ...
    def forward(self, similarity, teacher_score=None, subsampling_weight=None):
        ...
        k = getattr(self.args, 'sigmoid_teacher_masked_top_k', 0)
        
        if teacher_score is not None and k > 0:

            teacher_n_scores = teacher_score[:, 1:]
            num_neg_samples = teacher_n_scores.shape[1]
            k = min(k, num_neg_samples - 1)

            _, top_k_indices = torch.topk(teacher_n_scores, k=k, dim=1)

            mask = torch.ones_like(student_n_score, dtype=torch.bool, device=student_n_score.device)
            
            mask.scatter_(dim=1, index=top_k_indices, value=0)
        else:
            mask = torch.ones_like(student_n_score, dtype=torch.bool, device=student_n_score.device)
...
class KL_divergency(nn.Module):
    ...
    def pre_process(self, stu_dis, tea_dis):
        ...
        if (self.tea_preprocess == 'local_standardize') and (not self.args.teacher_score_pre_softmax):
            tea_dis, _, _ = self.local_standardize(tea_dis)
        
        return stu_dis, tea_dis
    
    def forward(self, student_dis, teacher_dis, losstype='NormalTeacher', reduction='batchmean'):
        if losstype == 'NormalTeacher':
            student_dis, teacher_dis = self.pre_process(student_dis, teacher_dis)
            if self.args.teacher_score_pre_softmax:
                teacher_p = teacher_dis
            else:
...
class Total_Loss(nn.Module):
    def __init__(self, args):
        ...
        if args.hard_loss_function == 'SigmoidLoss':
            ...
            # self.stu_hard_loss = SimpleSigmoidLoss(args)
    ...
    def get_weights(self, head, relation, tail, stu_score, Euc_tea_score, Hyper_tea_score, data):
        ...
        if self.args.use_prior_weight: 
            prior_weight = self.prior_weight_learner(data)
            weights = (weights + self.args.prior_weight_weight * prior_weight)/(1+self.args.prior_weight_weight)
    ...
    def pre_softmax(self, Euc_tea_score, Hyper_tea_score):
        Euc_map_score = F.softmax(Euc_tea_score / self.temperature, dim=-1).detach()
        Hyper_tea_score = F.softmax(Hyper_tea_score / self.temperature, dim=-1).detach()
        return Euc_map_score, Hyper_tea_score
    ...
    def forward(self, stu_score, stu_embeddings, Euc_tea_score, Euc_embeddings, Hyper_tea_score, Hyper_embeddings, data, epoch):
        ...
        if self.args.teacher_score_pre_softmax:
            Euc_map_score, Hyper_map_score = self.pre_softmax(Euc_tea_score, Hyper_tea_score)
        else:
        ...
        if (self.args.single_teacher_learn_from_mapped_score) and (not self.args.teacher_score_pre_softmax):
            Euc_tea_score, Hyper_tea_score = Euc_map_score, Hyper_map_score
        
        if self.args.need_sigmoid_mask:
            hard_loss, hard_loss_record = self.stu_hard_loss(stu_score, teacher_score=Hyper_tea_score)
        else:
        ...
        Hyper_soft_loss = self.soft_loss(stu_score, Hyper_tea_score, losstype="NormalTeacher")
        ...
        if self.args.do_contrastive:
            contrasitve_loss = self.CL_loss(head, Euc_tea_head, Hyper_tea_head, weights)
        else:
            contrasitve_loss = torch.tensor(0.0, device=hard_loss.device, dtype=hard_loss.dtype)
        ...
        other_loss_record = {
            'Euc_soft_loss': Euc_soft_loss.item(),
            'Hyper_soft_loss': Hyper_soft_loss.item(),
            'Optimal_soft_loss': Optimal_soft_loss.item(),
            'Optimal_tea_loss': Optimal_tea_loss.item(),
            'contrastive_loss': contrasitve_loss.item(),
