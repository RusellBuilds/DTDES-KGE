...
class TrainDataset(Dataset):
    def __init__(self, triples, args):
        ...
        self.pre_sample_mode = args.pre_sample_mode
    ...
    def __getitem__(self, idx):
        ...
        if self.args.pre_sample_size == 0:
            ...
            if self.pre_sample_mode == 'teacher_Euc':
                pre_sample_list, forbidden_tails = self.pre_sampling1(head, relation, tail, pre_sample_num=self.args.pre_sample_size) # 从第一个教师模型采样
            elif self.pre_sample_mode == 'teacher_Hyper':
                pre_sample_list, forbidden_tails = self.pre_sampling2(head, relation, tail, pre_sample_num=self.args.pre_sample_size) # 从第二个教师模型采样
            elif self.pre_sample_mode == 'combined':
                pre_sample_list, forbidden_tails = self.pre_sampling3(head, relation, tail, pre_sample_num=self.args.pre_sample_size) # 从两个教师模型采样
            else:
                raise ValueError(f" pre_sample_mode: {self.pre_sample_mode}")
    ...
    def pre_sampling3(self, head, relation, tail, pre_sample_num=50):
        ...
        selected_tail = query_aware_tail[:pre_sample_num]
        return selected_tail, query_aware_tail
